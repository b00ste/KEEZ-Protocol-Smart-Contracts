...
import {
    ILSP6KeyManager
} from "@lukso/lsp-smart-contracts/contracts/LSP6KeyManager/ILSP6KeyManager.sol";
 ...
 * @title Delegates
 */
contract Delegates is IDelegates {
    /**
     * @notice Address of the Universal Profile.
     */
    address payable private UNIVERSAL_PROFILE;

    /**
     * @notice Address of the Key Manager.
     */
    address private KEY_MANAGER;

    /**
     * @dev Address of the Universal Governance Structure.
     */
    address private UNIVERSAL_GOVERNANCE_STRUCTURE;
    ...
    constructor(
        ...
        address _UNIVERSAL_GOVERNANCE_STRUCTURE
    ) {
        UNIVERSAL_PROFILE = _UNIVERSAL_PROFILE;
        KEY_MANAGER = _KEY_MANAGER;
        UNIVERSAL_GOVERNANCE_STRUCTURE = _UNIVERSAL_GOVERNANCE_STRUCTURE;
    }

    // --- General Methods.

    /**
     * @inheritdoc IDelegates
     */
    function delegate(address delegatee) external override {
        ...
        keys[0] = bytes32(
            bytes.concat(_DAO_DELEGATEE_PREFIX(UNIVERSAL_GOVERNANCE_STRUCTURE), bytes20(msg.sender))
        );
        ...
        keys[1] = bytes32(
            bytes.concat(
                _DAO_DELEGATES_ARRAY_PREFIX(UNIVERSAL_GOVERNANCE_STRUCTURE),
                bytes20(delegatee)
            )
        );
     ...
     * @inheritdoc IDelegates
     */
    function changeDelegate(address newDelegatee) external override {
        _verifyPermission(msg.sender, _PERMISSION_SEND_DELEGATE, "SEND_DELEGATE");
        ...
        values[1] = _removeAddressFromArray(
            msg.sender,
     ...
     */
    function _getPermissions(address _from) internal view returns (bytes32 permissions) {
        permissions = bytes32(
            IERC725Y(UNIVERSAL_PROFILE).getData(
                bytes32(
                    bytes.concat(
                        _UGS_PARTICIPANTS_ARRAY_KEY(UNIVERSAL_GOVERNANCE_STRUCTURE),
                        bytes20(_from)
                    )
                )
            )
     ...
     */
    function _setDataMultiple(bytes32[] memory _keys, bytes[] memory _values) internal {
        ILSP6KeyManager(KEY_MANAGER).execute(
            abi.encodeWithSelector(setDataMultipleSelector, _keys, _values)
        );
    }
     ...
     */
    function _addAddressToArray(address newElement, bytes memory enocdedAddressArray)
        internal
        pure
        returns (bytes memory updatedEncodedAddressArray)
    {
        if (enocdedAddressArray.length != 0) {
            ...
            bytes memory upadtedLength = bytes.concat(
                bytes32(uint256(bytes32(delegatesArrayLength)) + 1)
            );
     ...
     */
    function _removeAddressFromArray(address removedElement, bytes memory enocdedAddressArray)
        internal
        pure
        returns (bytes memory updatedEncodedAddressArray)
    {
        ...
        if (bytes32array.length == 3) {
            ...
            for (uint256 i = 0; i < bytes32array.length - 1; ) {
                if (encodedRemovedElement == bytes32array[i]) {
                    updatedEncodedAddressArray = bytes.concat(
                        updatedEncodedAddressArray,
                        bytes32array[bytes32array.length - 1]
                    );
                } else if (i == 1) {
                    updatedEncodedAddressArray = bytes.concat(
                        updatedEncodedAddressArray,
                        bytes32(uint256(bytes32array[i]) - 1)
                    );
                } else {
                    updatedEncodedAddressArray = bytes.concat(
                        updatedEncodedAddressArray,
