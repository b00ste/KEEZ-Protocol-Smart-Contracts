...
/**
 *
 * @notice This smart contract is responsible for managing
 * the Universal Governance Structure Permissions Keys.
 *
 * @author B00ste
 * @title Permissions
 */
contract Permissions is IPermissions {
    ...
    /**
     * @dev The key for the participants of this Universal Governance Structure
     */
    bytes32 private UGS_PARTICIPANTS_ARRAY_KEY;
    ...
    constructor(
        ...
        UGS_PARTICIPANTS_ARRAY_KEY = _UGS_PARTICIPANTS_ARRAY_KEY(_UNIVERSAL_GOVERNANCE_STRUCTURE);
    }
    ...
    function getNewPermissionHash(
        ...
        _hash = keccak256(
            abi.encode(address(this), _from, _to, _permissions, claimPermissionNonce[_to])
        );
    }
     ...
     * @inheritdoc IPermissions
     */
    function addPermissions(address _to, bytes32 _permissions) external override {
     ...
     * @inheritdoc IPermissions
     */
    function removePermissions(address _to, bytes32 _permissions) external override {
    ...
    function _getPermissions(address _from) internal view returns (bytes32 permissions) {
        permissions = bytes32(
            IERC725Y(UNIVERSAL_PROFILE).getData(
                bytes32(
                    bytes.concat(bytes10(UGS_PARTICIPANTS_ARRAY_KEY), bytes2(0), bytes20(_from))
                )
            )
        );
    }
    ...
    function _addPermissions(address _to, bytes32 _permissions) internal {
        ...
        while (firstElementIndex < 32 && _permissions[firstElementIndex] != bytes1(0)) {
            unchecked {
                ++firstElementIndex;
            }
        }
        ...
        if (currentPermissions == bytes32(0))
            ArrayWithMappingLibrary._addElement(
                UNIVERSAL_PROFILE,
                KEY_MANAGER,
                UGS_PARTICIPANTS_ARRAY_KEY,
                bytes16(UGS_PARTICIPANTS_ARRAY_KEY),
                bytes10(UGS_PARTICIPANTS_ARRAY_KEY),
        ...
        ILSP6KeyManager(KEY_MANAGER).execute(
            abi.encodeWithSelector(
                setDataSingleSelector,
                bytes32(bytes.concat(bytes10(UGS_PARTICIPANTS_ARRAY_KEY), bytes20(_to))),
